...
def validate_json(data):
    """Validates the entire JSON structure."""
    if isinstance(data, list):
        return validate_questions_list(data)
    elif isinstance(data, dict):
        errors = []
        if 'questions' not in data:
            errors.append("Root object is missing 'questions' key.")
        else:
            errors.extend(validate_questions_list(data['questions']))
            ...
            if not isinstance(config, dict):
                ...
                if 'activeHours' in config:
                    ...
                    if not isinstance(ah, dict) or 'start' not in ah or 'end' not in ah:
                        errors.append("'activeHours' must have 'start' and 'end' keys.")
        return errors
    else:
        return ["JSON must be an array or an object."]
...
def generate_template():
    """Generates a sample questions.json with all 13 interaction types and config."""
    template = {
        "config": {
            "githubPingInterval": 60,
            "brainLockInterval": 30,
            "activeHours": {
                "start": "07:00",
                "end": "21:00"
            }
    ...
    if choice == '1':
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                data['questions'] = renumber_questions(data['questions'], start_id)
                output = data

            with open(filename, 'w', encoding='utf-8') as f:
                json.dump(output, f, indent=2)
            print("Successfully renumbered questions.")
        except Exception as e:
            print(f"Error: {e}")
        ...
        print("Generated 'questions_sample.json' with 13 interaction types and config.")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == '--test':
            ...
            renumbered_list = renumber_questions(template['questions'], 100)
            if renumbered_list[0]['id'] == 100:
